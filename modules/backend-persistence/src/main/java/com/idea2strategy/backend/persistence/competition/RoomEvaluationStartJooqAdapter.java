...
public class RoomEvaluationStartJooqAdapter implements RoomEvaluationStartPort {
    ...
    /** Registers period runs only after F has durably opened the evaluation ledger. */
    void registerCompetitionBacktests(UUID participationId, OffsetDateTime observedAt) {
        var participation = dsl.fetchOne(
                "select p.room_id, p.bot_id, r.competition_type::text as competition_type, "
                        + "p.status::text as status from competition.participations p "
                        + "join competition.rooms r on r.id = p.room_id where p.id = ? for update of p",
                participationId);
        if (participation == null
                || !"BACKTEST".equals(participation.get("competition_type", String.class))
                || !"EVALUATING".equals(participation.get("status", String.class))) {
            return;
        }
        insertCompetitionBacktestRequest(
                participation.get("room_id", UUID.class), participationId,
