    ...
    public Optional<BotExecutionPreflightFacts> findOwnedById(
        ...
        var bot = dsl.fetchOne(
                "select c.initial_cash_amount, "
                        ...
                        + "where waiting.bot_id = active.id and waiting.status = 'REGISTERED' "
                        ...
                        + "where reserved.owner_account_id = b.owner_account_id "
                        + "and reserved.status = 'REGISTERED' and reserved.evaluation_started_at is null "
                        + "and reserved_bot.deleted_at is null) "
                        + "+ case when b.lifecycle_status <> 'RUNNING' "
                        + "and not exists(select 1 from competition.participations target_reserved "
                        + "where target_reserved.bot_id = b.id and target_reserved.status = 'REGISTERED' "
                        + "and target_reserved.evaluation_started_at is null) then 1 else 0 end)::int "
                        + "as projected_count, "
