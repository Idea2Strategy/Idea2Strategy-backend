...
class OperatorBootstrapServiceTest {
    @Test
    void rejectsInvalidLoginName() {
        assertThatThrownBy(() -> service().execute(manifest("Not A Login", id(1)), "a".repeat(64)))
    ...
    void rejectsInitialRoleOutsideReviewedManifest() {
        assertThatThrownBy(() -> service().execute(manifest("admin", id(99)), "a".repeat(64)))
                .isInstanceOf(OperatorBootstrapRejectedException.class)
    ...
    private static OperatorBootstrapManifest manifest(String loginName, UUID initialRole) {
        UUID role = id(1); UUID permission = id(2);
        return new OperatorBootstrapManifest("bootstrap", "catalog", "d".repeat(64), "bootstrap_role",
                loginName, id(3), id(4), initialRole, id(5), "review-ticket-1", id(6), id(7),
