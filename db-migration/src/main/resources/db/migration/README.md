...
## Customer JWT refresh-family migration

`V20260807120000__backend_customer_refresh_token_families.sql` copies every existing
refresh credential into `identity.refresh_token_families`, changes customer strategy
edit leases from `session_id` to `account_id`, and only then drops `identity.sessions`.
The migration is forward-only because the removed device labels and last-seen values
are intentionally no longer product state. A whole-release rollback must restore the
pre-deployment database backup together with the previous backend/UI versions;
otherwise keep the migration applied and ship a timestamped forward fix.
